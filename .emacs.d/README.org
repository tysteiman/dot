...
Set this to C-M-x when in shell mode

#+begin_src elisp

  (define-key sh-mode-map (kbd "C-M-x") 'my/send-line-to-shell)
...
Basic helpers that can be extended for messing with using different monitors.

With using BSPWM, these could be extended to also shuffle around workspaces if needed.
...
These are small helper methods for setting bspwm configuration settings via bspc.
...
The idea is to be able to configure bspwm in Emacs via /(bspc-config "border_width 20")/ or something along those lines.
